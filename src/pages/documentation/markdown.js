...
import { Heading, Link, ListItem, Box } from "@chakra-ui/core";
...
const Markdown = ({content}) => {
    const RenderHeading = ({heading}) => {
        ...
        useLayoutEffect(() => {
          const inView = (target) => target.getBoundingClientRect().top > -40 && target.getBoundingClientRect().top < window.innerHeight / 2;
        ...
        return <>
            <Heading ref={ref} as={"h" + heading.level} mb="6" pt="12" id={id}>{heading.children}</Heading>
            <Fill name="Docs.menu">
              <ListItem mb="2"><Link href={'#' + id } fontWeight={actived ? "bold": "normal"}>{heading.children }</Link></ListItem>
            </Fill>
        </>
    }
